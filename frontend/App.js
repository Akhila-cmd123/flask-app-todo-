...
export default function TodoApp() {
  ...
  const fetchTasks = async () => {
    const response = await axios.get("http://43.204.150.192:5000/tasks");//in ec2 replace localhost with public ip of ec2
    setTasks(response.data);
  };

  const addTask = async () => {
    if (newTask.trim() === "") return;
    await axios.post("http://43.204.150.192:5000/tasks", { task: newTask });
    setNewTask("");
    fetchTasks();
  };

  const deleteTask = async (id) => {
    await axios.delete(`http://43.204.150.192:5000/tasks/${id}`);
    fetchTasks();
  };
